  ...
  module Geocoders
    class GoogleGeocoder3 < Geocoder
      ...
      def self.do_reverse_geocode(latlng)
        ...
        submit_url = submit_url("/maps/api/geocode/json?key=#{Geokit::Geocoders::google}&sensor=false&latlng=#{Geokit::Inflector::url_escape(latlng.ll)}")
      ...
      def self.do_geocode(address, options = {})
        ...
        submit_url = submit_url("/maps/api/geocode/json?key=#{Geokit::Geocoders::google}&sensor=false&address=#{Geokit::Inflector::url_escape(address_str)}#{bias_str}#{language_str}")
